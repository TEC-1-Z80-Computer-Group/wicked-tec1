...
interface Tec1FooterProps extends Stylable {
  classic: boolean;
  worker: any;
  handleChangeLayout: EventHandler;
}

const BaseTec1Footer = ({ classic, worker, handleChangeLayout, className }: Tec1FooterProps) =>{
...
return (
  <div className={`${className} tec1-footer`}>
    <div className={`${className} first-row`}>
      ...
      <div>
        <input id="key-layout" type="checkbox" value={classic} onChange={handleChangeLayout} />
        <label htmlFor="key-layout">original key layout</label>
      </div>
      <div>
        <label htmlFor="speed">Speed</label>
        Speed
        <input id="speed" type="range" min="0" max="99" value={speed || '50'} onChange={handleChangeSpeed} />
      </div>
    </div>
    <p>MON 1 Restart:</p>
